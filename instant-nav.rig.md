...
- RUN: `pnpm test:e2e` (Playwright, `./tests/e2e`, port 3100 default via `PORT`/`BASE_URL`). The config's `webServer` runs the build+start itself; for instant() runs use `EXPOSE_TESTING_API=1` in the environment so the webServer inherits it.
